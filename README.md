...
- 分类字段 top values 和占比。
- AI eval 失败分析：按状态、错误、类别、模型聚合 fail/error 样本。
- 时间字段范围、跨度统计。
- 抽样表，适合大文件快速扫视。
- 导出当前过滤结果 CSV。
- 导出 Markdown 摘要。
- 导出 Markdown 失败分析报告，方便贴到 PR、issue 或 eval 回归记录。
- localStorage 会话恢复。
- 示例数据可直接加载。
...
“失败分析”标签页会在当前过滤结果中识别常见失败信号：

- `status`、`result`、`outcome`、`state`、`verdict` 为 `fail`、`failed`、`error`、`timeout` 等。
- `success`、`ok`、`passed` 为 `false`。
- `error`、`errors`、`exception`、`failure_reason`、`error.message` 等字段非空。

识别后会按状态、错误原因、类别/套件、模型/代理聚合，并列出失败样本的行号、ID、分数和延迟。点击“导出失败报告”可以生成 Markdown drilldown，适合放进 PR 评论、CI 摘要或回归分析文档。
...
The Failures tab detects common eval failure signals such as `status=fail`, `outcome=error`, `success=false`, or non-empty `error` / `exception` / `failure_reason` / `error.message` fields. It groups matching rows by status, error, category or suite, and model or agent, then exports a Markdown drilldown for pull requests, CI summaries, or regression notes.
